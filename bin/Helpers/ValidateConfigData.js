...
exports.ValidateConfigData = void 0;
/**
...
async function ValidateConfigData(data) {
    ...
    if (data.sources[0]) {
        ...
        for (let item of data.sources) {
            ...
            if (!Array.isArray(item.fileExtensions)) {
                throw new Error(`Error with source item ${key}: fileExtensions is not an array.`);
            }
            if (item.fileExtensions.length >= 1) {
                item.fileExtensions.every((ext) => {
                    if (typeof ext !== "string") {
                        throw new Error(`Error with source item ${key}: fileExtensions ${ext} is not a string.`);
                    }
                });
            }
        }
    ...
    if (data.outputs[0]) {
        data.outputs.forEach((item, key) => {
            ...
            if (typeof item.dir !== "string" &&
                ...
                throw new Error(`Error with output item ${key}: dir is not a string or a string array.`);
            }
            ...
            if (typeof item.keepComments !== "boolean") {
                throw new Error(`Error with output item ${key}: keepComments is not a boolean.`);
            }
            if (!item.codeSection) {
                throw new Error(`Error with output item ${key}: no codesection provided.`);
            }
            if (Array.isArray(item.codeSection) &&
                typeof item.codeSection[0] !== "string") {
                throw new Error(`Error with output item ${key}: the codesection provided is not a string.`);
            }
            else if (typeof item.codeSection === "string") {
                const acceptable = ["all", "All", "None", "none"];
                if (acceptable.indexOf(item.codeSection) == -1) {
                    throw new Error(`Error with output item ${key}: the codesection  provided [${item.codeSection}] is invalid.`);
                }
            }
            else {
                ...
                item.codeSection.forEach((item) => {
                    if (typeof item !== "string") {
                        throw new Error(`Error with output item ${key}: the codesection provided [${item}] is not a string.`);
                    }
                    if (keys.indexOf(item) == -1) {
                        throw new Error(`Error with output item ${key}: the codesection provided [${item}] is not listed in the code section for the config file.`);
                    }
                });
            }
        });
    ...
    sections.forEach((item) => {
        ...
        if (!data.codeSections[item].start) {
            throw new Error(`Error with code section ${item}. Start is not provided.`);
        }
        else if (typeof data.codeSections[item].start !== "string") {
            throw new Error(`Error with code section ${item}. Start is not a string.`);
        }
